...
To prioritise chart data, add the `top_ranking_order`, `mojo_rank_order`, or `mojo_lifetime_gross_order` parameters. `top_ranking_order=asc` surfaces the lowest IMDb ranking numbers first (e.g., #1 at the top), while the Box Office Mojo parameters work the same way on `mojo.rank` and on the numeric value of `mojo.lifetime_gross`. When these parameters are present, items missing the requested metrics are filtered out. If you provide multiple sort options together, the precedence is `imdb.top_ranking`, then `mojo.rank`, then `mojo.lifetime_gross`, before falling back to the default popularity, ratings, and title ordering.
...
| mojo_lifetime_gross_order | asc,desc                                                                                                                                                                                                            | Sort by Box Office Mojo lifetime gross (items without data are excluded)                                  |
