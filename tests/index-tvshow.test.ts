...
const params = {
  ...
  correct_tmdb_id_returned_on_path_with_production_companies: {
    ...
    expectedResult: (item) => {
      ...
      expect(Array.isArray(item.production_companies)).toBe(true);
    },
  },
  ...
  correct_tmdb_id_returned_on_search: {
    ...
    expectedResult: (items) => {
      ...
      expect(items[0]).not.toHaveProperty("platforms_links");
      expect(items[0]).not.toHaveProperty("production_companies");
    },
  },

  correct_tmdb_id_returned_on_search_with_append_to_response: {
    query:
      "?tmdbid=249042&append_to_response=critics_rating_details,episodes_details,production_companies,directors,genres,networks,platforms_links",
    expectedResult: (items) => {
      expect(items.length).toBe(1);
      expect(items[0].id).toBe(249042);
      ...
      expect(items[0]).toHaveProperty("production_companies");
      expect(Array.isArray(items[0].production_companies)).toBe(true);
      expect(items[0]).toHaveProperty("platforms_links");
      expect(Array.isArray(items[0].platforms_links)).toBe(true);
    },
  },
  ...
  correct_tmdb_popularity_order: {
    query: `?item_type=tvshow&popularity_filters=tmdb_popularity&limit=${maxLimitLargeDocuments}`,
    expectedResult: (items) => {
  ...
  items_with_all_required_keys_active_tvshow: {
    query: `?item_type=tvshow&is_active=true&append_to_response=critics_rating_details,episodes_details,last_episode,next_episode,highest_episode,lowest_episode,production_companies,directors,genres,networks,platforms_links&limit=${maxLimitLargeDocuments}`,
    expectedResult: checkItemProperties,
  },

  items_with_all_required_keys_inactive_tvshow: {
    query: `?item_type=tvshow&is_active=false&append_to_response=critics_rating_details,episodes_details,last_episode,next_episode,highest_episode,lowest_episode,production_companies,directors,genres,networks,platforms_links&limit=${maxLimitLargeDocuments}`,
    expectedResult: checkItemProperties,
  },
  ...
  should_return_all_platforms: {
    query: `?item_type=tvshow&is_active=true,false&platforms=${encodeURIComponent("all,Disney+")}&append_to_response=platforms_links&limit=250`,
    expectedResult: (items) => {
  ...
  only_platforms_disney_plus: {
    query: `?item_type=tvshow&platforms=${encodeURIComponent("allWrong,Disney+")}&append_to_response=platforms_links&limit=250`,
    expectedResult: (items) => {
      expect(items.length).toBe(250);
  ...
  only_genres_drama_and_platforms_netflix: {
    query: `?item_type=tvshow&genres=${encodeURIComponent("Drama")}&platforms=${encodeURIComponent("Netflix")}&append_to_response=genres,platforms_links`,
    expectedResult: (items) => {
      items.forEach((item) => {
        expect(item).toHaveProperty("genres");
        expect(item).toHaveProperty("platforms_links");
  ...
  only_networks_hbo: {
    query: `?item_type=tvshow&networks=${encodeURIComponent("^hbo$")}&is_active=true,false&append_to_response=networks&limit=${maxLimit}`, // Added ‘HBO’ in lowercase and as a regex on purpose to ensure it works.
    expectedResult: (items) => {
  ...
  only_production_companies_paramount_pictures: {
    query: `?item_type=tvshow&production_companies=${encodeURIComponent("^paramount pictures$")}&is_active=true,false&append_to_response=production_companies,networks&limit=${maxLimit}`, // Added ‘Paramount Pictures’ in lowercase and as a regex on purpose to ensure it works.
    expectedResult: (items) => {
      expect(items.length).not.toBe(900);
  ...
  only_networks_and_production_companies_hbo: {
    query: `?item_type=tvshow&networks=${encodeURIComponent("^hbo$")}&production_companies=${encodeURIComponent("^hbo$")}&is_active=true,false&append_to_response=production_companies,networks&limit=${maxLimit}`,
    expectedResult: (items) => {
      expect(items.length).not.toBe(900);
  ...
  multiple_networks_and_companies: {
    query: `?item_type=tvshow&networks=${encodeURIComponent("hbo,netflix")}&production_companies=${encodeURIComponent("universal,warner")}&is_active=true,false&append_to_response=production_companies,networks&limit=${maxLimit}`,
    expectedResult: (items) => {
      expect(items.length).not.toBe(900);
  ...
  should_return_new_items_tvshow_when_filtered_by_release_date: {
    query: `?item_type=tvshow&is_active=true&release_date=everything,new&limit=${maxLimitLargeDocuments}`,
    expectedResult: (items) => {
      items.forEach((item) => {
  ...
  should_match_last_episode_details: {
    query: `?item_type=tvshow&is_active=true&append_to_response=episodes_details,last_episode&limit=${maxLimitLargeDocuments}`,
    expectedResult: (items) => {
      items.forEach((item) => {
  ...
  should_have_next_episode_greater_than_last_episode: {
    query: `?item_type=tvshow&is_active=true&append_to_response=last_episode,next_episode,episodes_details&limit=${maxLimitLargeDocuments}`,
    expectedResult: (items) => {
  ...
  should_have_increasing_episode_order_if_release_dates_differ: {
    query: `?item_type=tvshow&is_active=true&append_to_response=episodes_details&limit=${maxLimitLargeDocuments}`,
    expectedResult: (items) => {
      items.forEach((item) => {
        if (Array.isArray(item.episodes_details)) {
  ...
  should_not_have_next_episode_when_tvshow_is_ended: {
    query: `?item_type=tvshow&status=ended&is_active=true,false&append_to_response=episodes_details,next_episode&limit=${maxLimitLargeDocuments}`,
    expectedResult: (items) => {
      items.forEach((item) => {
        expect(item).toHaveProperty("status");
  ...
  should_have_highest_rated_episode_above_second_best: {
    query: `?item_type=tvshow&is_active=true,false&append_to_response=episodes_details,highest_episode&limit=${maxLimitLargeDocuments}`,
    expectedResult: (items) => {
      items.forEach((item) => {
  ...
  should_have_lowest_rated_episode_below_second_worst: {
    query: `?item_type=tvshow&is_active=true,false&append_to_response=episodes_details,last_episode,next_episode,highest_episode,lowest_episode&limit=${maxLimitLargeDocuments}`,
    expectedResult: (items) => {
      items.forEach((item) => {
        const all = item.episodes_details;
