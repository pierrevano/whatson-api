...
const checkDbIds = async (jsonArrayFromCSV, collectionData) => {
  ...
  try {
    const allDbData = await collectionData
      ...
      .map((el) => {
        return {
          ...
          thetvdbId: el.thetvdb ? el.thetvdb.id : null,
        };
      })
      .toArray();
    ...
    let dataToWrite = idsOnlyInDb
      .map((element) => {
        return `${element.allocineUrl},${element.imdbId},${element.betaseriesId},${element.tmdbId},${element.metacriticId},${element.rottenTomatoesId},${element.letterboxdId},${element.senscritiqueId},${element.traktId},${element.thetvdbId},FALSE\n`;
      })
      .join("");
