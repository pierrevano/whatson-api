...
To prioritise chart data, add the `top_ranking_order`, `mojo_rank_order`, or `mojo_lifetime_gross_order` parameters. `top_ranking_order=asc` surfaces the lowest IMDb ranking numbers first (e.g., #1 at the top). `mojo_rank_order=asc` keeps the same behaviour on `mojo.rank`, while `mojo_lifetime_gross_order=asc` surfaces the highest lifetime grosses first. Use `mojo_lifetime_gross_order=desc` if you need to see the lowest grosses first. The `mojo.lifetime_gross` values are returned as numbers expressed in US dollars. When these parameters are present, items missing the requested metrics are filtered out. If you provide multiple sort options together, the precedence is `imdb.top_ranking`, then `mojo.rank`, then `mojo.lifetime_gross`, before falling back to the default popularity, ratings, and title ordering.
...
| mojo_lifetime_gross_order | asc,desc                                                                                                                                                                                                            | Sort by Box Office Mojo lifetime gross (`asc` = highest first). Values are numbers in USD; items without data are excluded. |
