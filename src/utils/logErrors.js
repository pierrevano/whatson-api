...
const logErrors = (error, item, origin) => {
  ...
  if (error.response && error.response.status >= 500) {
    ...
    if (item && !(item.startsWith(config.baseURLBetaseriesFilm) || item.startsWith(config.baseURLBetaseriesSerie))) {
      process.exit(1);
    }
  }
  ...
  if (isErrorPresent(errorMsg, error, item)) {
    ...
    if (errorMsg && !(errorMsg.startsWith(config.baseURLBetaseriesFilm) || errorMsg.startsWith(config.baseURLBetaseriesSerie))) {
      process.exit(1);
    }
  }
};
